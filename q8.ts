...
function commonBoth(firstArray : number[], secondArray : number[]){
   const maxLengthArray: number[] = (firstArray.length > secondArray.length) ? firstArray : secondArray;
   const minLengthArray: number[]= (firstArray.length > secondArray.length) ? secondArray: firstArray;
   let arr : number[]=[];
   for(const num of minLengthArray){
      if(maxLengthArray.includes(num)){
       arr.push(num);
      }
   }
   return arr;
}
console.log(commonBoth([1,2,334,5,6],[1,2,5]));
